...
fn help_commands_exit_successfully() {
    ...
    for args in help_cases {
        ...
        assert!(
            ...
            "herdr-dumb {} failed: status={:?} stdout={} stderr={}",
            args.join(" "),
            output.status.code(),
    ...
    for args in [&["--help"][..], &["agent", "--help"][..]] {
        ...
        for expected in [
            ...
            assert!(
                ...
                "herdr-dumb {} help did not contain {expected:?}: {stdout}",
                args.join(" ")
            );
        }
    }
...
fn removed_wait_and_agent_send_commands_are_rejected() {
    let wait = Command::new(env!("CARGO_BIN_EXE_herdr-dumb"))
    ...
    let help = Command::new(env!("CARGO_BIN_EXE_herdr-dumb"))
        .arg("--help")
        .output()
        .unwrap();
    assert!(!String::from_utf8_lossy(&help.stdout).contains("herdr-dumb wait <subcommand>"));

    let send = Command::new(env!("CARGO_BIN_EXE_herdr-dumb"))
        .args(["agent", "send", "reviewer", "hello"])
        .output()
        .unwrap();
    assert_eq!(send.status.code(), Some(2));
    let stderr = String::from_utf8_lossy(&send.stderr);
    assert!(stderr.contains("herdr-dumb agent send-keys"));
    assert!(!stderr.contains("herdr-dumb agent send <"));
}
...
fn completion_command_prints_zsh_script_without_session_startup() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("#compdef herdr-dumb"), "stdout: {stdout}");
    assert!(
        !stdout.contains("--cwd=[]"),
...
fn root_help_hides_explicit_client_command() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        !stdout.contains("herdr-dumb client"),
...
fn root_help_advertises_api_schema_command_group() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("herdr-dumb api <subcommand>"),
...
fn api_schema_default_output_is_a_short_summary() {
    ...
    assert!(
        stdout.contains("Use `herdr-dumb api schema --json`"),
        "stdout: {stdout}"
    );
    assert!(
        stdout.len() < 400,
...
fn explicit_client_command_respects_nested_guard() {
    ...
    assert!(
        stderr.contains("nested herdr-dumb is disabled by default"),
...
fn removed_show_changelog_flag_fails_before_nested_guard() {
    ...
    assert!(
        !stderr.contains("nested herdr-dumb"),
