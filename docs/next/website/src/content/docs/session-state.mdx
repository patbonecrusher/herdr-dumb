...
Live handoff is for update flows that need to replace a running Herdr server. It asks the old server to transfer live panes to the new server, so pane processes can keep running across the server replacement.
...
```bash
herdr-dumb server live-handoff
```
