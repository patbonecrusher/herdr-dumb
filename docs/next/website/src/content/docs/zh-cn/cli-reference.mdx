...
```bash
herdr-dumb                         # 启动或连接默认会话
herdr-dumb --default-config        # 打印默认配置
herdr-dumb completion zsh          # 生成 zsh 补全脚本
herdr-dumb --version               # 打印版本
```
...
`herdr-dumb api schema` 会打印安装的二进制中包含的 socket 协议 schema 简短摘要。需要完整 JSON Schema 文档时使用 `--json`;要写入文件则使用 `--output PATH`。
...
`herdr-dumb server` 显式运行无界面服务器,适合被监管或服务式的部署。`reload-config` 在不重启窗格的情况下应用可重载设置。`agent-manifests` 显示生效的智能体检测清单来源、缓存的远程版本和最近的远程更新结果。`reload-agent-manifests` 在编辑本地覆盖后,把智能体检测清单重载到运行中的服务器。
