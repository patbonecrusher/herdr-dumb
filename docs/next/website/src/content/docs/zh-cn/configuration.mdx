...
运行 `herdr-dumb --help` 可查看系统解析后的配置路径。
...
主题、侧边栏和复制行为使用本地客户端配置。窗格默认值、集成和自定义命令使用本地服务器配置。UI 中的 `reload config` 会重载两者。
...
如果你已有旧的自定义按键绑定，并想使用新的默认值，请运行 `herdr-dumb config reset-keys`。Herdr 会备份 `config.toml`，移除 `[keys]` 和 `[[keys.command]]`，并在重启或执行 `herdr-dumb server reload-config` 后使用内置的 v2 默认值。
...
- `machine` — 为兼容旧配置保留的 token；在仅本地会话中省略。
- `workspace` — 工作区名称。
- `tab` — 标签页名称（如有）。
- `pane` — 窗格名称（如有）。
...
token 会按配置顺序渲染。Herdr 通常使用 ` · ` 分隔相邻值，并在 `state_icon` 后使用一个空格。缺失值及其分隔符会消失；当一行中的所有 token 都没有值时，该行会消失。已有的自定义行不会改变。每个布局最多可包含 16 行，每行最多可包含 16 个 token。
...
只有第一条匹配的规则生效。指定的样式字段覆盖当前位置的默认值，未指定的字段保持继承。没有规则匹配时保留默认样式。匹配使用显示截断前的完整值。在规则中设置 `hide = true` 可移除匹配的 token 及其分隔符；没有剩余 token 的行也会消失。例如，`{ token = "workspace", fg = "#61afef", rules = [{ equals = "main", hide = true }] }` 会在工作区名称恰好为 `main` 时隐藏工作区 token。设置 `hide = false` 或省略时，token 保持可见。没有覆盖字段的规则一旦匹配，仍会停止后续匹配并保留默认值。
...
更改此设置后，需要重启受影响的 Herdr 服务器或重新连接客户端。
