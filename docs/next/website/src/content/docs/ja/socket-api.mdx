...
クライアント描画の Herdr UI は、ローカルサーバーとの接続に安定したエンドポイント世代を使います。クライアントとサーバーのビルドが一致する必要はありません。接続時にスナップショット、画面、入力、blob コーデックを合意し、対応 API メソッドと任意の能力を通知します。メソッドがない場合は該当アクションだけを無効にし、拒否やタイムアウトは接続を終了せず通知します。エンドポイント世代 1 より前のサーバーは更新が必要です。
