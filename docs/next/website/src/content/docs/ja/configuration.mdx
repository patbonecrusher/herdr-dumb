...
システム上で解決された設定ファイルのパスは、`herdr-dumb --help` で確認できます。
...
テーマ、サイドバー、コピー動作はローカルクライアントの設定を使います。ペインの既定値、インテグレーション、カスタムコマンドはローカルサーバーの設定を使います。UI の `reload config` は両方をリロードします。
...
古いカスタムキーバインドを使っていて新しいデフォルトが欲しい場合は、`herdr-dumb config reset-keys` を実行してください。Herdr は `config.toml` をバックアップし、`[keys]` と `[[keys.command]]` を削除し、再起動または `herdr-dumb server reload-config` の後に組み込みの v2 デフォルトを使います。
...
- `machine` — 互換性のために残されたトークン。ローカル専用セッションでは省略されます。
- `workspace` — ワークスペース名。
- `tab` — 利用可能な場合はタブ名。
- `pane` — 利用可能な場合はペイン名。
...
トークンは設定された順序で描画されます。Herdr は通常、隣接する値を ` · ` で区切り、`state_icon` の後には空白を 1 つ入れます。値がない場合、その値と区切りは表示されません。すべてのトークンに値がない行は表示されません。既存のカスタム行は変更されません。各レイアウトは最大 16 行、各行は最大 16 トークンです。
...
この設定を変更した場合は、対象の Herdr サーバーを再起動するか、クライアントを再アタッチする必要があります。
