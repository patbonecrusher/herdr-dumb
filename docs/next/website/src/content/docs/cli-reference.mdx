...
```bash
herdr-dumb                         # launch or attach to the default session
herdr-dumb --default-config        # print default config
herdr-dumb completion zsh          # generate a zsh completion script
herdr-dumb --version               # print version
```
...
`herdr-dumb api schema` prints a short summary of the socket protocol schema bundled
...
herdr-dumb pane input [<pane_id>|--pane ID|--current] --right-click herdr|pane
...
mouse-reporting pane application. `herdr-dumb` restores the default pane menu.
...
herdr-dumb integration install grok
...
herdr-dumb integration uninstall grok
