...
`current_tab_id`, and may only be compared to the matching ID field.
...
The client-rendered Herdr UI connects to a local server using a stable endpoint
generation. Client and server builds do not need to match. During connection setup,
they agree on snapshot, screen, input, and blob codecs, and the server advertises
supported API methods and optional capabilities. Missing methods disable only
the corresponding actions. Rejections and timeouts show a notice without ending
the connection. Servers from before endpoint generation 1 need one final update.
