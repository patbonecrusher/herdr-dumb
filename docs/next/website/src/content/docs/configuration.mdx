...
```text
Linux and macOS: ~/.config/herdr-dumb/config.toml
Windows:          %APPDATA%\herdr\config.toml
```

Run `herdr-dumb --help` to see the resolved config path for your system.
...
```bash
herdr-dumb --default-config > ~/.config/herdr-dumb/config.toml
```
...
Themes, sidebar layout, and copy behavior use the local client's config. Pane defaults, integrations, and custom commands use the local server's config. The UI's `reload config` action reloads both.
...
If you have old custom keybindings and want the new defaults, run `herdr-dumb config reset-keys`. Herdr backs up `config.toml`, removes `[keys]` and `[[keys.command]]`, and uses built-in v2 defaults after restart or `herdr-dumb server reload-config`.
...
tab_bar_right = [
  ...
  { type = "text", text = "prod" },
  { type = "command", command = "~/.config/herdr-dumb/status.sh", interval_seconds = 5, timeout_seconds = 2 },
]
...
- `machine` — legacy compatibility token; omitted in local-only sessions.
...
Changing this setting requires restarting the affected Herdr server or reattaching the client.
...
```text
~/.config/herdr-dumb/herdr.log
~/.config/herdr-dumb/herdr-client.log
~/.config/herdr-dumb/herdr-server.log
```
