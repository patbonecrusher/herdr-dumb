...
fn tab_get(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_tab_id) = args.first() else {
        eprintln!("usage: herdr-dumb tab get <tab_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: herdr-dumb tab get <tab_id>");
...
fn tab_focus(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_tab_id) = args.first() else {
        eprintln!("usage: herdr-dumb tab focus <tab_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: herdr-dumb tab focus <tab_id>");
...
fn tab_rename(args: &[String]) -> std::io::Result<i32> {
    if args.len() < 2 {
        eprintln!("usage: herdr-dumb tab rename <tab_id> <label>");
        return Ok(2);
    }
...
fn tab_close(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_tab_id) = args.first() else {
        eprintln!("usage: herdr-dumb tab close <tab_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: herdr-dumb tab close <tab_id>");
...
fn print_tab_help() {
    eprintln!("herdr-dumb tab commands:");
    eprintln!("  herdr-dumb tab list [--workspace <workspace_id>]");
    eprintln!(
        "  herdr-dumb tab create [--workspace <workspace_id>] [--cwd PATH] [--label TEXT] [--env KEY=VALUE] [--focus] [--no-focus]"
    );
    eprintln!("  herdr-dumb tab get <tab_id>");
    eprintln!("  herdr-dumb tab focus <tab_id>");
    eprintln!("  herdr-dumb tab rename <tab_id> <label>");
    eprintln!("  herdr-dumb tab close <tab_id>");
}
