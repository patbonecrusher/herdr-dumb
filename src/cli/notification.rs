...
fn notification_show(args: &[String]) -> std::io::Result<i32> {
    let params = match parse_notification_show_args(args) {
        ...
        Err(NotificationShowArgError::Usage) => {
            eprintln!(
                "usage: herdr-dumb notification show <title> [--body TEXT] [--position top-left|top-right|bottom-left|bottom-right] [--sound none|done|request]"
            );
            return Ok(2);
        }
...
fn print_notification_help() {
    eprintln!("herdr-dumb notification commands:");
    eprintln!(
