...
fn pane_get(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_pane_id) = args.first() else {
        eprintln!("usage: herdr-dumb pane get <pane_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: herdr-dumb pane get <pane_id>");
...
fn parse_pane_neighbor_args(args: &[String]) -> Result<PaneNeighborParams, String> {
    ...
    let Some(direction) = direction else {
        return Err(
            "usage: herdr-dumb pane neighbor --direction left|right|up|down [--pane ID|--current]"
                .into(),
        );
    };
...
fn parse_pane_focus_args(args: &[String]) -> Result<PaneFocusDirectionParams, String> {
    let params = parse_pane_neighbor_args(args).map_err(|_| {
        "usage: herdr-dumb pane focus --direction left|right|up|down [--pane ID|--current]"
            .to_string()
    })?;
...
fn parse_pane_resize_args(args: &[String]) -> Result<PaneResizeParams, String> {
    ...
    let Some(direction) = direction else {
        return Err(
            "usage: herdr-dumb pane resize --direction left|right|up|down [--amount FLOAT] [--pane ID|--current]"
                .into(),
        );
    };
...
fn pane_rename(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_pane_id) = args.first() else {
        eprintln!("usage: herdr-dumb pane rename <pane_id> <label>|--clear");
        return Ok(2);
    };
    if args.len() < 2 {
        eprintln!("usage: herdr-dumb pane rename <pane_id> <label>|--clear");
...
fn parse_pane_read_args(args: &[String]) -> Result<PaneReadParams, String> {
    const USAGE: &str = "usage: herdr-dumb pane read <pane_id> [--source visible|recent|recent-unwrapped|detection] [--lines N] [--format text|ansi] [--ansi] [--raw]";
...
fn parse_pane_input_args(
    ...
    const USAGE: &str =
        "usage: herdr-dumb pane input [<pane_id>|--pane ID|--current] --right-click herdr|pane";
...
fn parse_pane_split_args(
    ...
    let Some(direction) = direction else {
        return Err(
            "usage: herdr-dumb pane split [<pane_id>|--pane ID|--current] --direction right|down [--ratio FLOAT] [--cwd PATH] [--env KEY=VALUE] [--right-click herdr|pane] [--focus] [--no-focus]"
                .into(),
        );
    };
...
fn parse_pane_swap_args(args: &[String]) -> Result<PaneSwapParams, String> {
    ...
    match (directional, explicit) {
        ...
        _ => Err(
            "usage: herdr-dumb pane swap --direction left|right|up|down [--pane ID|--current]\n       herdr-dumb pane swap --source-pane ID --target-pane ID"
                .into(),
        ),
    }
}
...
fn pane_close(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_pane_id) = args.first() else {
        eprintln!("usage: herdr-dumb pane close <pane_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: herdr-dumb pane close <pane_id>");
...
fn pane_send_text(args: &[String]) -> std::io::Result<i32> {
    if args.len() < 2 {
        eprintln!("usage: herdr-dumb pane send-text <pane_id> <text>");
        return Ok(2);
    }
...
fn pane_send_keys(args: &[String]) -> std::io::Result<i32> {
    if args.len() < 2 {
        eprintln!("usage: herdr-dumb pane send-keys <pane_id> <key> [key ...]");
        return Ok(2);
    }
...
fn pane_run(args: &[String]) -> std::io::Result<i32> {
    if args.len() < 2 {
        eprintln!("usage: herdr-dumb pane run <pane_id> <command>");
        return Ok(2);
    }
...
fn parse_pane_wait_output_args(args: &[String]) -> Result<PaneWaitForOutputParams, String> {
    const USAGE: &str = "usage: herdr-dumb pane wait-output <pane_id> (--match TEXT | --regex PATTERN) [--source visible|recent|recent-unwrapped] [--lines N] [--timeout MS] [--raw]";
...
fn pane_report_agent(args: &[String]) -> std::io::Result<i32> {
    const USAGE: &str = "usage: herdr-dumb pane report-agent <pane_id> --source ID --agent LABEL --state idle|working|blocked|unknown [--message TEXT] [--seq N] [--agent-session-id ID] [--agent-session-path PATH]";
...
fn pane_report_agent_session(args: &[String]) -> std::io::Result<i32> {
    const USAGE: &str = "usage: herdr-dumb pane report-agent-session <pane_id> --source ID --agent LABEL [--seq N] [--agent-session-id ID] [--agent-session-path PATH] [--session-start-source SOURCE]";
...
fn pane_release_agent(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_pane_id) = args.first() else {
        eprintln!(
            "usage: herdr-dumb pane release-agent <pane_id> --source ID --agent LABEL [--seq N]"
        );
        return Ok(2);
    };
...
fn pane_report_metadata(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_pane_id) = args.first() else {
        eprintln!("usage: herdr-dumb pane report-metadata <pane_id> --source ID [--agent LABEL] [--applies-to-source ID] [--title TEXT|--clear-title] [--display-agent TEXT|--clear-display-agent] [--state-label STATUS=TEXT] [--clear-state-labels] [--token NAME=VALUE] [--clear-token NAME] [--seq N] [--ttl-ms N]");
        return Ok(2);
    };
...
fn print_pane_help() {
    eprintln!("herdr-dumb pane commands:");
    eprintln!("  herdr-dumb pane list [--workspace <workspace_id>]");
    eprintln!("  herdr-dumb pane current [--pane ID|--current]");
    eprintln!("  herdr-dumb pane get <pane_id>");
    eprintln!("  herdr-dumb pane layout [--pane ID|--current]");
    eprintln!("  herdr-dumb pane process-info [--pane ID|--current]");
    eprintln!("  herdr-dumb pane neighbor --direction left|right|up|down [--pane ID|--current]");
    eprintln!("  herdr-dumb pane edges [--pane ID|--current]");
    eprintln!("  herdr-dumb pane focus --direction left|right|up|down [--pane ID|--current]");
    eprintln!(
        "  herdr-dumb pane resize --direction left|right|up|down [--amount FLOAT] [--pane ID|--current]"
    );
    eprintln!("  herdr-dumb pane zoom [<pane_id>|--pane ID|--current] [--toggle|--on|--off]");
    eprintln!("  herdr-dumb pane rename <pane_id> <label>|--clear");
    eprintln!("  herdr-dumb pane read <pane_id> [--source visible|recent|recent-unwrapped] [--lines N] [--format text|ansi] [--ansi]");
    eprintln!("  herdr-dumb pane input [<pane_id>|--pane ID|--current] --right-click herdr|pane");
    eprintln!(
        "  herdr-dumb pane split [<pane_id>|--pane ID|--current] --direction right|down [--ratio FLOAT] [--cwd PATH] [--env KEY=VALUE] [--right-click herdr|pane] [--focus] [--no-focus]"
    );
    eprintln!("  herdr-dumb pane swap --direction left|right|up|down [--pane ID|--current]");
    eprintln!("  herdr-dumb pane swap --source-pane ID --target-pane ID");
    eprintln!("  herdr-dumb pane move <pane_id> --tab <tab_id> --split right|down [--target-pane ID] [--ratio FLOAT] [--focus|--no-focus]");
    eprintln!("  herdr-dumb pane move <pane_id> --new-tab [--workspace ID] [--label TEXT] [--focus|--no-focus]");
    eprintln!("  herdr-dumb pane move <pane_id> --new-workspace [--label TEXT] [--tab-label TEXT] [--focus|--no-focus]");
    eprintln!("  herdr-dumb pane close <pane_id>");
    eprintln!("  herdr-dumb pane send-text <pane_id> <text>");
    eprintln!("  herdr-dumb pane send-keys <pane_id> <key> [key ...]");
    eprintln!("  herdr-dumb pane wait-output <pane_id> (--match TEXT | --regex PATTERN) [--source visible|recent|recent-unwrapped] [--lines N] [--timeout MS] [--raw]");
    eprintln!("  herdr-dumb pane report-agent <pane_id> --source ID --agent LABEL --state idle|working|blocked|unknown [--message TEXT] [--seq N] [--agent-session-id ID] [--agent-session-path PATH]");
    eprintln!("  herdr-dumb pane report-agent-session <pane_id> --source ID --agent LABEL [--seq N] [--agent-session-id ID] [--agent-session-path PATH]");
    eprintln!("  herdr-dumb pane release-agent <pane_id> --source ID --agent LABEL [--seq N]");
    eprintln!("  herdr-dumb pane report-metadata <pane_id> --source ID [--agent LABEL] [--applies-to-source ID] [--title TEXT|--clear-title] [--display-agent TEXT|--clear-display-agent] [--state-label STATUS=TEXT] [--clear-state-labels] [--token NAME=VALUE] [--clear-token NAME] [--seq N] [--ttl-ms N]");
    eprintln!("  herdr-dumb pane run <pane_id> <command>");
}
...
mod tests {
    ...
    fn parse_pane_swap_args_rejects_mixed_forms() {
        ...
        assert!(err.contains("usage: herdr-dumb pane swap"));
    }
    ...
    fn parse_pane_move_args_rejects_target_pane_without_tab() {
        ...
        assert!(err.contains("usage: herdr-dumb pane move"));
    }
