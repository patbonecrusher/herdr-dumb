...
fn agent_explain(args: &[String]) -> std::io::Result<i32> {
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "help" | "--help" | "-h" => {
                eprintln!("usage: herdr-dumb agent explain <target> [--json|--verbose]");
                eprintln!(
                    "usage: herdr-dumb agent explain --file PATH --agent LABEL [--json|--verbose]"
                );
                return Ok(0);
            }
            ...
            value => {
                if target.is_some() {
                    eprintln!("usage: herdr-dumb agent explain <target> [--json]");
                    return Ok(2);
                }
    ...
    let explain = if let Some(path) = file {
        if target.is_some() {
            eprintln!("usage: herdr-dumb agent explain --file PATH --agent LABEL [--json]");
            return Ok(2);
        }
        let Some(agent_label) = agent else {
            eprintln!("herdr-dumb agent explain --file requires --agent LABEL");
            return Ok(2);
        };
...
fn agent_start(args: &[String]) -> std::io::Result<i32> {
    let Some(name) = args.first() else {
        eprintln!("usage: herdr-dumb agent start <name> --kind KIND --pane ID [--timeout MS] [-- <agent-args...>]");
        return Ok(2);
    };
    let separator = args
        .iter()
...
fn agent_list(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: herdr-dumb agent list");
        return Ok(2);
    }
...
fn agent_get(args: &[String]) -> std::io::Result<i32> {
    let Some(target) = args.first() else {
        eprintln!("usage: herdr-dumb agent get <target>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: herdr-dumb agent get <target>");
...
fn agent_focus(args: &[String]) -> std::io::Result<i32> {
    let Some(target) = args.first() else {
        eprintln!("usage: herdr-dumb agent focus <target>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: herdr-dumb agent focus <target>");
...
fn agent_attach(args: &[String]) -> std::io::Result<i32> {
    let (target, takeover) = match super::parse_attach_target(
        args,
        "usage: herdr-dumb agent attach <target> [--takeover]",
    ) {
        Ok(parsed) => parsed,
...
fn agent_wait(args: &[String]) -> std::io::Result<i32> {
    let Some(target) = args.first() else {
        eprintln!("usage: herdr-dumb agent wait <target> [--until STATUS]... [--timeout MS]");
        return Ok(2);
    };
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "help" | "--help" | "-h" => {
                eprintln!(
                    "usage: herdr-dumb agent wait <target> [--until STATUS]... [--timeout MS]"
                );
                return Ok(0);
            }
            other => {
...
fn agent_rename(args: &[String]) -> std::io::Result<i32> {
    let [target, value] = args else {
        eprintln!("usage: herdr-dumb agent rename <target> <name>|--clear");
        return Ok(2);
    };
...
fn agent_prompt(args: &[String]) -> std::io::Result<i32> {
    let Some(target) = args.first() else {
        eprintln!(
            "usage: herdr-dumb agent prompt <target> <text> [--wait] [--until STATUS]... [--timeout MS]"
        );
        return Ok(2);
    };
...
fn agent_send_keys(args: &[String]) -> std::io::Result<i32> {
    if args.len() < 2 {
        eprintln!("usage: herdr-dumb agent send-keys <target> <key> [key ...]");
        return Ok(2);
    }
...
fn agent_read(args: &[String]) -> std::io::Result<i32> {
    let Some(target) = args.first() else {
        eprintln!("usage: herdr-dumb agent read <target> [--source visible|recent|recent-unwrapped] [--lines N] [--format text|ansi] [--ansi]");
        return Ok(2);
    };
...
fn print_agent_help() {
    eprintln!("herdr-dumb agent commands:");
    eprintln!("  herdr-dumb agent list");
    eprintln!("  herdr-dumb agent get <target>");
    eprintln!("  herdr-dumb agent read <target> [--source visible|recent|recent-unwrapped|detection] [--lines N] [--format text|ansi] [--ansi]");
    eprintln!("  herdr-dumb agent send-keys <target> <key> [key ...]");
    eprintln!(
        "  herdr-dumb agent prompt <target> <text> [--wait] [--until STATUS]... [--timeout MS]"
    );
    eprintln!("  herdr-dumb agent rename <target> <name>|--clear");
    eprintln!("  herdr-dumb agent focus <target>");
    eprintln!("  herdr-dumb agent wait <target> [--until STATUS]... [--timeout MS]");
    eprintln!("  herdr-dumb agent attach <target> [--takeover]");
    eprintln!(
        "  herdr-dumb agent start <name> --kind KIND --pane ID [--timeout MS] [-- <agent-args...>]"
    );
    eprintln!("  herdr-dumb agent explain <target> [--json|--format text|json] [--verbose]");
    eprintln!(
        "  herdr-dumb agent explain --file PATH --agent LABEL [--json|--format text|json] [--verbose]"
    );
