...
fn api_snapshot(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: herdr-dumb api snapshot");
        return Ok(2);
    }
...
fn schema_summary_text() -> std::io::Result<String> {
    ...
    Ok(format!(
        "Herdr API schema\nprotocol: {}\nschema_version: {}\nschemas: {}\n\nUse `herdr-dumb api schema --json` to print the full schema.\nUse `herdr-dumb api schema --output PATH` to write it to a file.\n",
        protocol,
        schema_version,
        schemas.join(", ")
    ))
...
fn print_api_help() {
    eprintln!("herdr-dumb api commands:");
    eprintln!("  herdr-dumb api snapshot");
    eprintln!("  herdr-dumb api schema [--json | --output PATH]");
}

fn print_api_schema_help() {
    eprintln!("usage: herdr-dumb api schema [--json | --output PATH]");
}
...
mod tests {
    ...
    fn schema_summary_text_stays_human_sized() {
        ...
        assert!(text.contains("Use `herdr-dumb api schema --json`"));
        assert!(text.len() < 400);
    }
}
