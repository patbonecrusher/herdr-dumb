...
pub(super) fn run_completion_command(args: &[String]) -> std::io::Result<i32> {
    ...
    if matches!(shell, Shell::Zsh) {
        ...
        generate(shell, &mut command, "herdr-dumb", &mut output);
        ...
        crate::platform::begin_cli_output();
        generate(shell, &mut command, "herdr-dumb", &mut std::io::stdout());
    }
    Ok(0)
}
...
fn print_completion_help() {
    eprintln!(
        "usage: herdr-dumb completion <{}>",
        supported_shells_usage()
    );
}
