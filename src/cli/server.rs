...
fn server_stop(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: herdr-dumb server stop");
        return Ok(2);
    }
...
fn server_reload_config(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: herdr-dumb server reload-config");
        return Ok(2);
    }
...
fn server_agent_manifests(args: &[String]) -> std::io::Result<i32> {
    let json = match args {
        ...
        _ => {
            eprintln!("usage: herdr-dumb server agent-manifests [--json]");
            return Ok(2);
        }
    };
...
fn server_reload_agent_manifests(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: herdr-dumb server reload-agent-manifests");
        return Ok(2);
    }
...
fn server_live_handoff(args: &[String]) -> std::io::Result<i32> {
    let Some(params) = parse_live_handoff_params(args) else {
        eprintln!(
            "usage: herdr-dumb server live-handoff [--import-exe <path>] [--expected-protocol <n>] [--expected-version <version>]"
        );
        return Ok(2);
    };
...
fn print_server_help() {
    eprintln!("herdr-dumb server commands:");
    eprintln!("  herdr-dumb server                run as headless server");
    eprintln!("  herdr-dumb server stop           stop the running server via the API socket");
    eprintln!("  herdr-dumb server live-handoff   hand off live panes to a new local server");
    eprintln!("  herdr-dumb server reload-config  reload config.toml in the running server");
    eprintln!("  herdr-dumb server agent-manifests [--json]  show agent detection manifest status");
    eprintln!("  herdr-dumb server reload-agent-manifests  reload agent detection manifests in the running server");
}
