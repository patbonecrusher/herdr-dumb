...
pub fn run_server() -> io::Result<()> {
    ...
    let _api_server = match api::start_server_with_stop_control(
        ...
        Err(err) if err.kind() == io::ErrorKind::AddrInUse => {
            eprintln!("error: herdr-dumb server is already running");
...
fn print_ready_message(api_socket: &Path, client_socket: &Path) {
    eprintln!(
        "herdr-dumb server running; you can use any herdr-dumb CLI command in another terminal."
    );
    ...
    eprintln!("did you mean to open the Herdr TUI? run `herdr-dumb`; you do not need `herdr-dumb server`.");
}
