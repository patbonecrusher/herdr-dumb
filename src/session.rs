...
pub fn configure_from_args(args: &[String]) -> Result<Vec<String>, String> {
        ...
        && args.get(2).map(String::as_str) == Some("attach")
    {
        ...
        let Some(name) = args.get(3) else {
            return Err("usage: herdr-dumb session attach <name>".to_string());
        };
        if args.len() != 4 {
            return Err("usage: herdr-dumb session attach <name>".to_string());
        }
...
pub fn local_attach_command() -> String {
    match active_name() {
        Some(name) => format!("herdr-dumb session attach {name}"),
        None => "herdr-dumb".to_string(),
    }
}
...
pub fn stop_command_for(name: Option<&str>) -> String {
    match name {
        Some(name) => format!("herdr-dumb session stop {name}"),
        None => "herdr-dumb server stop".to_string(),
    }
}
...
fn exact_session_dir_for_delete(name: &str) -> Result<Option<PathBuf>, String> {
    ...
    match std::fs::symlink_metadata(sessions_dir.join(name)) {
        Ok(_) => Err(format!(
            "session {name} does not match an exact session name; use the spelling shown by `herdr-dumb session list`"
        )),
...
mod tests {
    ...
    fn local_attach_command_uses_default_launch_for_default_session() {
        ...
        assert_eq!(local_attach_command(), "herdr-dumb");
    }
    ...
    fn local_attach_command_uses_session_attach_for_named_session() {
        ...
        assert_eq!(local_attach_command(), "herdr-dumb session attach work");
    ...
    fn local_stop_command_uses_server_stop_for_default_session() {
        ...
        assert_eq!(local_stop_command(), "herdr-dumb server stop");
    ...
    fn local_stop_command_uses_session_stop_for_named_session() {
        ...
        assert_eq!(local_stop_command(), "herdr-dumb session stop work");
    ...
    fn restart_after_update_guidance_names_stop_and_attach_commands() {
        assert_eq!(
            restart_after_update_guidance(
                "herdr-dumb session stop work",
                Some("herdr-dumb session attach work")
            ),
            "Stop the old server to use the new version.\nStopping exits pane processes.\nRun `herdr-dumb session stop work`, then run `herdr-dumb session attach work` again."
        );
    }
    ...
    fn active_restart_after_update_guidance_respects_socket_override() {
        ...
        assert_eq!(
            ...
            "Stop the old server to use the new version.\nStopping exits pane processes.\nRun `HERDR_SOCKET_PATH=/tmp/custom-herdr.sock herdr-dumb server stop`, then restart Herdr with the same socket override."
        );
